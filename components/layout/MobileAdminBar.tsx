...
import { X, Users, Bell, BarChart2, GraduationCap, LogOut, Calendar, CalendarDays, CalendarClock, LayoutGrid, Plug, MessageSquare, Megaphone, Home, Satellite, MoreHorizontal, ClipboardList, ClipboardCheck, ShieldAlert, Network, UserPlus, Users2, Banknote, FolderOpen, Heart, UsersRound } from 'lucide-react'
...
const nav = [
  ...
  { href: '/admin/coursework', label: 'Coursework', icon: ClipboardCheck },
  ...
  { href: '/admin/youth', label: 'Youth', icon: Users2 },
  { href: '/admin/safeguarding', label: 'Safeguarding', icon: ShieldAlert, teacherHidden: true },
  { href: '/admin/wellbeing', label: 'Wellbeing', icon: Heart },
  { href: '/admin/bursaries', label: 'Bursaries', icon: Banknote, teacherHidden: true },
      ...
      <aside className={`md:hidden fixed top-0 left-0 z-50 h-[100dvh] w-72 bg-tranmere-blue text-white flex flex-col shadow-2xl transform transition-transform duration-300 ease-out ${open ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto mt-2 pb-4">
          {visibleNav.map(({ href, label, icon: Icon, external }) => {
            ...
            const className = `flex items-center gap-3 px-3 py-3 rounded-lg text-sm transition-colors ${active ? 'bg-white/20 font-semibold' : 'active:bg-white/10 text-blue-100'}`
            return external ? (
