...
export default async function StudentDetailPage({ params }: { params: { id: string } }) {
  ...
  const [
    ...
    supabase
      ...
      .select('id, term, scheduled_for, status, completed_at, ai_summary')
      .eq('student_id', studentId)
        ...
        <div className="rounded-2xl border bg-white p-5 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="font-semibold flex items-center gap-1.5">
              <ClipboardList size={16} className="text-tranmere-blue" /> Learner Reviews
            </h2>
            <Link
              href={`/admin/students/${student.id}/review/new?name=${encodeURIComponent(student.name ?? '')}`}
              className="inline-flex items-center gap-1 text-xs font-semibold text-tranmere-blue bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded-lg"
            >
          ...
          {!reviews || reviews.length === 0 ? (
            <p className="text-sm text-muted-foreground">No reviews yet. Start the first 1-to-1.</p>
          ) : (
            <ul className="space-y-2">
              {reviews.map(r => (
                <li key={r.id} className="flex items-start justify-between gap-2 border-b last:border-0 py-2.5">
                  <div className="space-y-0.5">
                    <p className="text-sm font-medium">{r.term}</p>
                    {r.scheduled_for && (
                      <p className="text-xs text-muted-foreground">
                        {new Date(r.scheduled_for).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
                      </p>
                    )}
                    {(r.ai_summary as { text?: string } | null)?.text && (
                      <p className="text-xs text-muted-foreground line-clamp-2 mt-1 max-w-xs">
                        {(r.ai_summary as { text: string }).text.slice(0, 120)}…
                      </p>
                    )}
                  </div>
